...
{ 
  // 1. 为即将打开的文件所对应的locked_FILE对象申请空间。
  struct locked_FILE
  {
    struct _IO_FILE_plus fp;
#ifdef _IO_MTSAFE_IO
    _IO_lock_t lock;  // amdt64下有该成员
#endif
...
#endif
  // 2. 初始_IO_FILE对象file各成员，指针均置NULL，设置通用flags
  _IO_no_init (&new_f->fp.file, 0, 0, &new_f->wd, &_IO_wfile_jumps);
  // 3. 设置vtable指向全局对象_IO_file_jumps
  _IO_JUMPS (&new_f->fp) = &_IO_file_jumps;
  // 4. 为新打开文件，设定特定参数（fileno=-1等），并将_IO_FILE_plus对象插入全局_IO_list_all链表头部（头插法）
  _IO_new_file_init_internal (&new_f->fp);
  // _IO_file_fopen 实际等于 _IO_new_file_fopen, 位于fileops.c。打开文件，记录文件描述符，成功则再次尝试插入_IO_list_all.
  if (_IO_file_fopen ((FILE *) new_f, filename, mode, is32) != NULL)
    return __fopen_maybe_mmap (&new_f->fp.file);
  // 如果失败，则从_IO_list_all头部摘下new_f->fp
