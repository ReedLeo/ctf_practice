...
{
  if ((fp->file._flags & _IO_LINKED) == 0)  // 检查是否已经插入链表，避免重复插入
    {
      fp->file._flags |= _IO_LINKED;  // 标记当前对象为已插入链表
#ifdef _IO_MTSAFE_IO  //  调试结果来看，存在该宏，即下列代码均存在。
      _IO_cleanup_region_start_noarg (flush_cleanup);
      _IO_lock_lock (list_all_lock);
      run_fp = (FILE *) fp;
      _IO_flockfile ((FILE *) fp);
#endif
      fp->file._chain = (FILE *) _IO_list_all;  // 头插法入链
      _IO_list_all = fp;
#ifdef _IO_MTSAFE_IO
      _IO_funlockfile ((FILE *) fp);
      run_fp = NULL;
...
__underflow (FILE *fp)
{
  ...
  // 当前读缓存已无数据，调用libio/fileops.c:_IO_new_file_underflow(FILE* fp)
...
{ // 原先有缓冲区，且_flags未标记为”使用用户缓存“，则释放之。
  if (f->_IO_buf_base && !(f->_flags & _IO_USER_BUF))
    free (f->_IO_buf_base);
  f->_IO_buf_base = b;
  f->_IO_buf_end = eb;
  if (a) // 调用者决定是否清除"使用用户缓存空间”标志位
...
{
  if (fp->_IO_buf_base) // 已经分配过了，不重复分配
    return;
    // _flags未打上“无缓存“标记 或 _mode>0 则尝试分配
  if (!(fp->_flags & _IO_UNBUFFERED) || fp->_mode > 0)
    if (_IO_DOALLOCATE (fp) != EOF) // 调用_vtable里的_IO_file_doallocate()申请堆块作为缓冲区
      return;
    // 到此，说明动态内存申请失败 或 无需动态缓冲区。 直接用fp内部的char _shortbuf[1]作为缓冲区
...
_IO_old_init (FILE *fp, int flags)
{
  ...
  fp->_markers = NULL;
  fp->_cur_column = 0;
#if _IO_JUMPS_OFFSET  // 该宏存在
  fp->_vtable_offset = 0;
#endif
#ifdef _IO_MTSAFE_IO  // 该宏存在
  if (fp->_lock != NULL)
	     ...
	     struct _IO_wide_data *wd, const struct _IO_jump_t *jmp)
{
  ...
  if (orientation >= 0)
    { // 初始化宽字符(wide_char版的_IO_FILE_plus对象)
      fp->_wide_data = wd;
