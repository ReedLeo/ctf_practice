...
_IO_new_file_init_internal (struct _IO_FILE_plus *fp)
{
  ...
  _IO_link_in (fp);      // 插入_IO_list_all链表头部
  fp->file._fileno = -1; // 初始化文件描述符为-1
}
...
{
  int fdesc;
  // 一般调用的是__open。Linux下就是系统调用open
  if (__glibc_unlikely (fp->_flags2 & _IO_FLAGS2_NOTCANCEL))
    fdesc = __open_nocancel (filename,
			     posix_mode | (is32not64 ? 0 : O_LARGEFILE), prot);
  else
    fdesc = __open (filename, posix_mode | (is32not64 ? 0 : O_LARGEFILE), prot);
  if (fdesc < 0)        // open()打开文件失败
    return NULL;
  fp->_fileno = fdesc;  // 记录文件描述符到FILE对象中
  // 更新_flags，根据read_write参数记录当前FILE对象是否存在读/写者，是否是追加(append)模式。
  _IO_mask_flags (fp, read_write,_IO_NO_READS+_IO_NO_WRITES+_IO_IS_APPENDING);
  /* For append mode, send the file offset to the end of the file.  Don't
     update the offset cache though, since the file handle is not active.  */
  if ((read_write & (_IO_IS_APPENDING | _IO_NO_READS))
      == (_IO_IS_APPENDING | _IO_NO_READS))
    {   // 若为追加模式，且无读者，则将当前写位置移到文件尾。
  ...
  // 再次尝试插入_IO_list_all，而_flags中存在_IO_LINKED可以避免重复插入
		    ...
		    int is32not64)
{
  ...
  // fp是否已关联已打开文件描述符(_fileno != -1)，是则直接返回
  // 因为一个FILE对象只能关联一个文件描述符
  if (_IO_file_is_open (fp))
    return 0;
  switch (*mode)
    {   // 根据mode来设置flags
  ...
  for (i = 1; i < 7; ++i)
	...
	case 'x':
	  ...
	  last_recognized = mode;
	  continue;
	case 'b':       // POSIX 忽略"b"模式，这与windows不同
	  last_recognized = mode;
	  continue;
	case 'm':
...
_IO_new_file_underflow (FILE *fp)
{
  ...
  if (fp->_IO_buf_base == NULL)
      ...
      if (fp->_IO_save_base != NULL)
	{
	  ...
	  fp->_flags &= ~_IO_IN_BACKUP;
        ...
        // 没有缓冲区，就分配一个，记录在[_IO_buf_base, _IO_buf_end)中
      _IO_doallocbuf (fp);
    }
  ...
  /* This is very tricky. We have to adjust those
     ...
     // 更新读/写缓冲区到_IO_buf_base
  fp->_IO_read_base = fp->_IO_read_ptr = fp->_IO_buf_base;
  fp->_IO_read_end = fp->_IO_buf_base;
  fp->_IO_write_base = fp->_IO_write_ptr = fp->_IO_write_end
    = fp->_IO_buf_base;
  // 系统调用read()一次性最多读取一个缓冲区大小的数据
  ...
  if (fp->_IO_buf_base == NULL)
      ...
      if (fp->_IO_save_base != NULL)
	{
	  ...
	  fp->_flags &= ~_IO_IN_BACKUP;
        ...
        // 没有分配缓冲区，则分配一个。记录缓冲区位置[_IO_buf_base, _IO_buf_end)
      _IO_doallocbuf (fp);
    }
  // 直到读够所需字符数为止
  while (want > 0)
	  ...
	  if (fp->_IO_buf_base)
	    {
	      ...
	      if (block_size >= 128)    // 读前先向按缓冲区大小下对齐一次读取的数量。
		count -= want % block_size;
	    }
          // 所需字符数量大于整个缓冲区大小，则直接使用read()读取
