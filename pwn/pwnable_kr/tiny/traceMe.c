...
void catflag(pid_t pid) {
    ...
    puts("start poker path to child...");
    for (int i = 0; i < sizeof(path) / sizeof(int); ++i) {
        if (-1 == ptrace(PTRACE_POKEUSER, pid, remote_path+i*sizeof(int), &((int*)path)[i])) {
            perror("poker failed.");
        }
    }
    ptrace(PTRACE_SINGLESTEP, pid, 0, 0);
    wait(NULL);
    for (int i = 0; i < sizeof(path_read) / sizeof(int); ++i) {
        if (-1 == ptrace(PTRACE_PEEKTEXT, pid, remote_path+i*sizeof(int), &((int*)path_read)[i])) {
            perror("poker failed.");
        }
    }
    printf("read the path from remote: %s\n", path_read);
    // wait(NULL);
