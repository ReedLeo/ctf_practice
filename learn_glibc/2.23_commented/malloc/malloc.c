...
static struct malloc_par mp_ =
{
  ...
  .mmap_threshold = DEFAULT_MMAP_THRESHOLD, // 128K
  .trim_threshold = DEFAULT_TRIM_THRESHOLD, // 128K
...
sysmalloc (INTERNAL_SIZE_T nb, mstate av)
{
   ...
   * 1. 堆没有初始化(av == NULL) 
   * 或者
   * 2.1 请求的大小大于等于事先设定的mmap阈值 (nb >= mp_.mmap_threshold)
   * 2.2 并且仍有mmap页面可以分配(mp_.n_mmaps < mp_.n_mmaps_max)
   * -----------------------------
   * ps: 默认情况下
   * static struct malloc_par mp_ =
   *  {
  ...
  */
  if (av == NULL
    ...
    try_mmap:
      ...
      // nb已经在_int_malloc函数中按chunk要求对齐了。如果直接使用mmap分配内存的话
      // 该chunk就不存在nextchunk.prev_size可以复用，所以还需要加上额外的SIZE_SZ
      ...
      // 如果+SIZE_SZ并对齐后size <= nb 则说明发生了溢出，则不分配内存
          ...
          if (mm != MAP_FAILED)
            {
              ...
              // 如果新的mmap分配内存块数量大于最大数量，则更新最大数量
              // 实际上，不会发生更新操作。因为mmap分配条件中就包括了
              // mp_.n_mmaps < mp_.n_mmaps_max
   ...
   */
  // 1. 如果第一次调用该函数，top chunk没有初始化。此时，av->bins[]中所有元素均为零
  // 2.1 如果已经初始化了，那么top chunk至少有MINSIZE那么大
  // 2.2 且 top chunk的前一个chunk必需标记为使用中
  // 2.3 且 top chunk的结束地址必需是页对齐的
  ...
  if (av != &main_arena)
    {
      ...
      // 定位sub_heap的hep_info实例
      old_heap = heap_for_ptr (old_top);
      old_heap_size = old_heap->size;
      /**
       *  1. 申请的内存大小没有溢出
       *  2. grow_heap()扩展堆成功
       *  则更新heap_info以及top.size
       */
      ...
      /**
       * new_heap初始化一个新堆， 堆大小=nb+(MINSIZE + sizeof(*heap))
       * 1. nb, 申请chunk的大小
       * 2. MINSIZE, 保留的最小chunk
       * 3. struct heap_info的大小， 因为新堆需要保存heap_info的信息。
       * 但主堆不需要heap_info，因为主堆直接查找全局变量main_arena
       */ 
    ...
    { /* Request enough space for nb + pad + overhead */
       ...
       */
      // 一般情况下，主分区用sbrk()中获取连续虚拟内存作为heap
      // 所以这里先减去top chunk已有的空闲分区大小
      if (contiguous (av))
        size -= old_size;
       ...
       */
      // 检查对齐后是否溢出; 函数开头定义 long size; size是有符号的
      if (size > 0)
        {
          // 没有溢出， 向系统申请更多空间
          // 默认情况下， MORECORE即sbrk
      ...
      if (brk != (char *) (MORECORE_FAILURE))
        ...
        { // srbk申请内存失败
          ...
          /**
           * 如果内存连续，前面就把top chunk的大小减去了。
           * 但前面连续内存扩展失败，只能在不连续的地址上分配增量内存
           * 且不再用原top chunk中的空间了，要重新把old_size加回去
           */
          if (contiguous (av))
            size = ALIGN_UP (size + old_size, pagesize);

          /* If we are relying on mmap as backup, then use larger units */
          // 默认情况下，mmap分配每次至少分配1MB空间
          // 默认 MMAP_AS_MORECORE_SIZE = 1M
      ...
      if (brk != (char *) (MORECORE_FAILURE))
        { // sbrk或mmp分配成功，更新sbrk基址，当前arena已分配堆内存大小；
          if (mp_.sbrk_base == 0)
            mp_.sbrk_base = brk;
          av->system_mem += size;
           ...
           */
          // sbrk成功在连续空间上直接扩展了top，则更新top.size数据
          // sbrk若成功，返回申请空间的起始位置
          if (brk == old_end && snd_brk == (char *) (MORECORE_FAILURE))
            set_head (old_top, (size + old_size) | PREV_INUSE);
          /**
           * 当前arena所分配的堆内存标记为连续的，
           * 即是使用sbrk而非MMAP来扩展堆的
           * 且原top.size非零
           * brk却小于原堆的结束地址，说明出错了
           */
           ...
           */
          /**
           * 走到这里，说明sbrk()返回的值大于原top chunk的结束地址
           * 因此新的地址和原top chunk的地址就不连续。可能是其他地方
           * 调用了sbrk()造成的。
           * 因此，处理对齐问题
           */ 
          else
            {
              ...
              if (contiguous (av))
                { // 当前arena管理的堆内存是连续的， 即通过sbrk扩展的
                  /* Count foreign sbrk as system_mem.  */
                  if (old_size)
                    // 实际新增管理内存要去掉本来就空闲的top.size部分
                    // 中间可能包含了外部调用sbrk所扩展的内存，也一并
                    // 计入当前arena管理的堆中
                   ...
                   */
                  // 因为top chunk与brk已经不相邻了，故不能使用原top chunk的空间
                  // 需要在新内存区域给原top chunk留一个同样大小的位置
                  correction += old_size;

                  /* Extend the end address to hit a page boundary */
                  // 从brk开始重新分配size+correction的空间
                  // 所以brk+size+correction就是新的内存区域结束地址
                  // 不过这个结束地址是未经对齐的
                  end_misalign = (INTERNAL_SIZE_T) (brk + size + correction);
                  correction += (ALIGN_UP (end_misalign, pagesize)) - end_misalign;
                  // correction 没有溢出
                  assert (correction >= 0);
                  // 将内存区域上界扩展correction个字节
                  // snd_brk是?
                  ...
                  if (snd_brk == (char *) (MORECORE_FAILURE))
                    { // 修正上界扩展失败， 获取当前上界
                      correction = 0;
                      // 无论是否扩展成功, snd_brk都指向了当前arena所辖堆内存的上界
              ...
              /* handle non-contiguous cases */
                ...
                { // 当前arena所辖堆内存非连续存放， 即使用了MMAP扩展过堆
                  if (MALLOC_ALIGNMENT == 2 * SIZE_SZ)
              ...
              if (snd_brk != (char *) (MORECORE_FAILURE))
                { // 结束地址合法， 则更新top.size和arena所辖堆内存大小
...
systrim (size_t pad, mstate av)
{
   ...
   */
  /**
   * 当且仅当目前brk==av->top + top_size时，才收缩
   * 这是为了避免我们释放了外部sbrk调用申请的内存
   */
...
_int_malloc(mstate av, size_t bytes)
{
   ...
   */
    ...
    {
        // 根据nb计算下标。注意，宏fastbin_index在计算前会把nb强转为unsigned。
        // x64下，这会导致截断，只使用了nb的低四字节。（x64下，size_t nb 占8字节）
        // 故我们可以令fake_fastbin_chunk.size = 0xaaaaaaaa0000007f，从而成功
        // 绕过 fastbin_index(chunksize(victim)) != idx 的检查
        ...
        do
        {
            ...
            if (victim == NULL)
                break;
        } // 从对应的fastbin_chunk单链表中摘取第一个节点
        while ((pp = catomic_compare_and_exchange_val_acq(fb, victim->fd, victim)) != victim);
        // 摘取的节点有效（非空）
        if (victim != 0)
        {
            // 检查所取chunk.size是否合法
            // 注意! fastbin_index(sz) 先将((unsigned int)sz)，在x64下会产生截断
   ...
   */
    ...
    if (in_smallbin_range(nb))
    {
        ...
        // FIFO, 最先入队列的chunk位于双向链表的末尾，故用last(bin)
        if ((victim = last(bin)) != bin)
        {
            // 当且仅当堆初始化前bins[]成员均为0。
            // 故检测到victim==0，意味着要初始化堆
            ...
            { // smallbin中存在空闲chunk
                ...
                bck = victim->bk;
                // 检查bkc->fd == victim，防止伪造chunk
                if (__glibc_unlikely(bck->fd != victim))
                {
                    errstr = "malloc(): smallbin double linked list corrupted";
                    goto errout;
                }
                // 设置nextchunk.size.prev_inuse = 1
                // 表示当前chunk已使用
                set_inuse_bit_at_offset(victim, nb);
                // 从当前bin的双向链表中摘除已分配的chunk
                bin->bk = bck;
                bck->fd = bin;

                // 当前不在main_arena表示的堆中分配内存
                if (av != &main_arena)
   ...
   */
    ...
    {
        // 计算下标前，nb强转为unsigned long。 因为sizeof(long)在x32下为4，
        // 在x64下为8，等于sizeof(size_t)，故不会发生截断。
        idx = largebin_index(nb);
        // 当前arena中使用了有空闲的fastbin，则合并之。
   ...
   */
    ...
    for (;;)
    {
        ...
        /* 
            处理unsorted_bin
        */
        // FIFO双向链表（队列），取出第一个入队的chunk
        while ((victim = unsorted_chunks(av)->bk) != unsorted_chunks(av))
        {
            bck = victim->bk;
            // 检查取出的chunk大小是否合法; 
            // 不能过小，也不能过大; 一般，system_mem=132K
           ...
           */
            ...
            */
                ...
                (unsigned long)(size) > (unsigned long)(nb + MINSIZE))
            {
                ...
                {   // 清空largebin的nextsize指针
                    remainder->fd_nextsize = NULL;
                    remainder->bk_nextsize = NULL;
                }
                // 设置victim.size中的大小、标志位
                set_head(victim, nb | PREV_INUSE |
                                     (av != &main_arena ? NON_MAIN_ARENA : 0));
                set_head(remainder, remainder_size | PREV_INUSE);
                // 因为remainder是空闲的，所以要在其后一个chunk设置prev_size
            ...
            */
            ...
            { // 属于largebin
              // largin_index计算前将size强转为unsigned long
              // sizeof(unsigned long) == sizeof(size_t)在x32和x64恒成立，
              // 故不会像fastbin或smallbin的下标计算一样产生截断
                ...
                if (fwd != bck)
                {
                    ...
                    // bck是largebin的头节点，largebin是有序的，bck->bk是当前largebin中最小的chunk
                    // 若申请的大小比当前最小的largebin还要小，则插入到队首
                    ...
                    {   // 即将插入的victim不是目标largebin最小的chunk
                        // 则从FIFO队尾开始遍历，从最大的chunk开始比较，寻找合适位置插入
                        assert((fwd->size & NON_MAIN_ARENA) == 0);
                        while ((unsigned long)size < fwd->size)
                        {   // 没找到，指针向前移动到一个更小的largebin chunk
                            fwd = fwd->fd_nextsize;
                            assert((fwd->size & NON_MAIN_ARENA) == 0);
                        }

                        // 找到相同大小的chunk，总是插入第二个位置
                        if ((unsigned long)size == (unsigned long)fwd->size)
                            /* Always insert in the second position.  */
                            fwd = fwd->fd;
                        else
                        {   
                            // 比bck.size小，比fwd.size大
                    ...
                    // 目标largebin是空的，直接插入即可
                    victim->fd_nextsize = victim->bk_nextsize = victim;
            }

            // 在bitmap中标记该bin中有空闲chunk可以分配
...
#define MAX_ITERS 10000
            // unsortedbin的回收尝试只会进行MAX_INTERS次
            if (++iters >= MAX_ITERS)   
                break;
        }   // unsortedbin 处理结束; 遍历过却未分配的unsortedbin都移到其对应的bin中
       ...
       */
        ...
        if (!in_smallbin_range(nb))
        {
          // idx 在for循环之外计算， 若不是smallbin的分配请求
          // idx必为largebin的下标
            bin = bin_at(av, idx);

            /* skip scan if empty or largest chunk is too small */
            // 1. largebin非空
            // 2. 且 largebin最大的chunk满足申请
            // 则查询最合适的chunk (best fit)
            if ((victim = first(bin)) != bin &&
                (unsigned long)(victim->size) >= (unsigned long)(nb))
            {
                // 从最小的chunk开始查找
                // 一开始victim指向最大的chunk，
                // 故victim->bk_nextsize指向了最小的chunk
                victim = victim->bk_nextsize;
                // 循环查找第一个满足chunk.size>=nb的chunk
                while (((unsigned long)(size = chunksize(victim)) <
                        (unsigned long)(nb)))
                    victim = victim->bk_nextsize;
                // 循环结束，victim指向可分配nb的最小chunk

                /* Avoid removing the first entry for a size so that the skip
                 list does not have to be rerouted.  */
                // 判断头节点(bin[])的size是没有义意的。为什么？回顾malloc_state.bins[]的实现
                // 如果满足要求、相同大小的chunk有多个，总是选用下一个
                if (victim != last(bin) && victim->size == victim->fd->size)
                    victim = victim->fd;
                // 计算切分后的大小，注意unsigned long remainder_size
                remainder_size = size - nb;
                // 著名的unlink宏
                ...
                else
                {
                    ...
                    // 设置victim.size 以及低三位标志位(A|M|P)
                    // 注意：victim.size必然有PREV_INUSE, 因为victim
                    // 相邻的低地址chunk一定在使用中，否则在free时就合并了
                    set_head(victim, nb | PREV_INUSE |
                                         (av != &main_arena ? NON_MAIN_ARENA : 0));
                    set_head(remainder, remainder_size | PREV_INUSE);
                    set_foot(remainder, remainder_size);
       ...
       */
      /**
       * 走到这里，说明当前的请求的大小无法在其对应的bin中找到chunk为之分配
       * 转而向更大的fastbin、smallbin或largebin中取寻找合适的chunk
      */
        ++idx;  // 指向下一个更大的bin
        bin = bin_at(av, idx);
        block = idx2block(idx);
        // 获取当前map表示的32个bin中是否有空闲chunk
        map = av->binmap[block];
        // bit表示所需chunk所在的bin
        bit = idx2bit(idx);
        ...
        for (;;)
        {
            ...
            /**
            * bit > map 表示所需的chunk的bin排在现有非空bin之后（即已有空闲块太小）
            * bit == 0 默认情况下不可能，因为idx2bit="((1U << ((i) & ((1U << BINMAPSHIFT) - 1))))"
            * 1左移任何位都非零。考虑到unsigned int bit是32位，是否有可能溢出而得到零呢？
            * 默认情况下，BINMAPSHIFT==5，((1U << BINMAPSHIFT) - 1)) == 31。所以展开宏后就是
            * bit = (1U << ((idx)&0x1f))， 故最多左移31位，不可能溢出；最少左移0位；
            * 无论哪种情况，bit结果都不可能为零
            */
            if (bit > map || bit == 0)
            {
                do
                { // 向后一个block进行查找，找到含有空闲chunk的block
                  // 或者找完整个bins[]都没有，就尝试使用top_chunk
                    if (++block >= BINMAPSIZE) /* out of bins */
                        goto use_top;
                } while ((map = av->binmap[block]) == 0);
                // 执行到此说明 map != 0, 该组block中必然含有空闲chunk的bin
                // 且此时的bin已经够大
                // 获取对应的bin
            ...
            while ((bit & map) == 0)
            {   // 当前bin中没有空闲块，向下一个更大的bin中寻找
                bin = next_bin(bin);
                // bit指向下一个bin
                bit <<= 1;
                // bit 不可能也不应该为零
                // unsgined bit 31次左移中必然找到非空bin
                // 因为前面的操作保证了map中包含了一个非空bin
                assert(bit != 0);
            }
            ...
            if (victim == bin)
            {
                // bitmap中的数据如果没错，则victim不应为空
                // 清空位图中的错误数据，将该bin标记为空
            ...
            {   // 成功从一个非空bin中取得一个够大的chunk
                size = chunksize(victim);

                /*  We know the first chunk in this bin is big enough to use. */
                // 前面的操作已经保证将使用的chunk.size必然足大
                ...
                {   // 切分后剩余空间还能构成一个合法chunk
                    // 将剩余空间构成的chunk插入到unsortedbin中
                    remainder = chunk_at_offset(victim, nb);
    ...
    use_top:
       ...
       */
        /**
         * 我们已经合并了所有的fastbin；查找了unsortedbin里的chunk，查找了现存bin
         * 都没有找到合适的足够大的chunk可以分配，只能使用top_chunk
         */
        victim = av->top;
        size = chunksize(victim);

        // 确保top.size切分后还剩下至少MINSIZE的空间
        if ((unsigned long)(size) >= (unsigned long)(nb + MINSIZE))
        {
            remainder_size = size - nb;
            remainder = chunk_at_offset(victim, nb);
        ...
        // top中的空间也不够了
        // 如果fastbin中存在chunk，则先合并一次fastbin中的chunk
        // 前面也有这段代码，但那只有在申请largebin时才会执行
        else if (have_fastchunks(av))
        {
            malloc_consolidate(av);
       ...
       */
        else  // top空间不够，fastbin中也没有空闲chunk可以合并了
        {
            void *p = sysmalloc(nb, av);
            if (p != NULL)
                alloc_perturb(p, bytes);
...
_int_free (mstate av, mchunkptr p, int have_lock)
{
  ...
  */
  ...
  if ((unsigned long)(size) <= (unsigned long)(get_max_fast ())
    ...
    // 待释放chunk的下一个chunk.size(至少含标志位PREV_INUSE) > MIN_CHUNK_SIZE
    // 且要小于当前arena所辖堆内存大小（av->system_mem）
    ...
    set_fastchunks(av);
    // x64下, size会被截断，只取低四字节。
    // fastbin_index计算前会把size强转为unsigned
    unsigned int idx = fastbin_index(size);
    // 取对应fastbin LIFO链表的头节点
    fb = &fastbin (av, idx);
  ...
  // free(A), free(B), free(A) 即可绕过
    ...
    // 到此，将当前chunk插入到对应fastbin的LIFO单链表中
  ...
  */
  // 处理非fastbin，也不是mmap分配的内存
    ...
    // 只检查了是否超过了arena所辖堆内存的上限
    // 如果修改了av->top.size，使得av->top + chunksize(av->top)溢出
    // 则可以绕过该检查
    ...
    // fasbin中的chunk不会情况prev_inuse标志位，所以该检查
    // 容易绕过
    ...
    // 如果与当待释放的chunk低地址方向相邻chunk已经标记为空闲
    // 则将p移动到低地址chunk上，并合并两个chunk为一个
    if (!prev_inuse(p)) {
    ...
    if (nextchunk != av->top) {
      ...
      */
      ...
      // 待释放chunk插入unsortedbin FIFO双向链表中
      p->fd = fwd;
      p->bk = bck;
      if (!in_smallbin_range(size))
      {
        // 如果是largebin，则清空两个nextsize指针
        p->fd_nextsize = NULL;
        p->bk_nextsize = NULL;
      }
      bck->fd = p;
      ...
      // 在下一个chunk上设置prev_size=size
      // 这句配合前面的consolidate forwad完成了与相邻高地址chunk的合并
      set_foot(p, size);
    ...
    */
     ...
     */
      ...
      av->top = p;
      check_chunk(av, p);
    } // fastbin, unsortedbin(smallbin, largebin) 处理完毕
    ...
    */
    /**
     * 对于通过mmap分配的大块内存
     * 如果size > FASTBIN_CONSOLIDATION_THRESHOLD(默认64K)
     * 则尝试合并fastbin
     */
